...
struct LoopView: View {
        ...
        Group {
            if showsCaption {
                loopStatus
                    ...
                    // Draws the solid capsule itself when idle and sweeps it while looping, so
                    // there is no separate `.overlay(Capsule())` here — a second static ring
                    // would sit on top of the travelling one and hide the sweep.
    ...
    private var loopAccessibilityLabel: String {
        ...
        let age: String
        // `showLooping`, not `isLooping`: the label has to describe the indicator that is on
    ...
    private var loopStatus: some View {
        HStack(alignment: .center) {
            ZStack {
                ...
                // Where there is a capsule, the sweeping border is what says "looping" and a
                // spinner in the ring would say it twice. The captionless modes have no
                // capsule, so there the ring centre carries it.
                if showLooping, !showsCaption {
                    ProgressView()
